...
**当前状态：不配置任何东西，API 就能跑** —— `ERA_AUTOMATION_EXECUTOR` 默认
...
| `ERA_AUTOMATION_DATABASE_URL` | env | 自动化控制面 DB，生产用 `postgresql+psycopg://…`（docker-compose 已注入） | 默认 sqlite 可跑但生产建议 PG |
| `ERA_AUTOMATION_WORKDIR` | env | evidence/工件工作目录（docker-compose 已注入 `/data/automation_work`，挂 volume） | 默认 `./automation_work` |
| vendor 嵌入技能 | `vendor/skills/` | anysearch / kimi-webbridge 等（Dockerfile 已 COPY） | 搜索适配器 fail-closed → BLOCKED |

## 第 2 层：真实研究编排（🟢 生产研究必需）

`ERA_AUTOMATION_EXECUTOR=orchestrating` 后按需配置：
...
| Kimi WebBridge | ① 二进制 `~/.kimi-webbridge/bin/kimi-webbridge`；② daemon 运行（`kimi-webbridge start`，默认 127.0.0.1:10086）；③ 浏览器扩展已连接；④ `ERA_KIMI_WEB_SESSION`（默认 `default`） | 三者任一缺失 → health 不可用 → 该适配器被过滤（不阻断 AnySearch）。注意：daemon 在宿主机时**容器内无法访问宿主 127.0.0.1**，容器部署下深度轮次（产品页）会 blocked——产品覆盖不完整会以 WARNING 呈现 |
...
| 配置项 | 说明 | 缺失后果 |
|---|---|---|
| `ERA_FEISHU_APP_ID` / `ERA_FEISHU_APP_SECRET` | 飞书开放平台自建应用凭证 | 通知 no-op（日志记录），研究不受影响 |
| `ERA_FEISHU_DEFAULT_RECEIVER` | 接收人 email / open_id / chat_id | 同上 |
...
| 配置项 | 说明 |
|---|---|
| n8n 服务 | `docker compose up` 已含（:5678）；导入 `automation/n8n/energy-research-agent-workflow.json` |
...
| 配置 | 读入点（代码） |
|---|---|
| `ERA_AUTOMATION_DATABASE_URL` | `automation/api/app.py:create_app` |
| `ERA_AUTOMATION_WORKDIR` | 同上 |
| `ERA_AUTOMATION_EXECUTOR` | `automation/api/app.py:_default_executor` |
| `ERA_KIMI_WEB_SESSION` | `automation/orchestration.py:from_environment` |
| `ERA_FEISHU_*` | `automation/feishu/lark.py` + `notifier.py` |
| `ERA_DEEPSEEK_*` / `ERA_OPENAI_*` | `settings.py` → `gateway/litellm_gateway.py` |
| `ERA_SKILL_ROOT` | `vendor.py:repository_root`（覆盖 Agent 与 vendor 根目录定位） |
