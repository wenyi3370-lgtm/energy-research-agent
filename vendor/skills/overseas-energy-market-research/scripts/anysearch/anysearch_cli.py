...
def _call_api(tool_name: str, arguments: dict, api_key: str) -> str:
    ...
    try:
        try:
            ...
            # Preserve a working user proxy, but recover when the inherited
            # Windows/environment proxy cannot establish the TLS connection.
            # The retry uses the same AnySearch endpoint and does not mutate
            # system proxy settings. (与官方 3.0.1 CLI 一致的直连回退；
            # ProxyError 是 ConnectionError 子类，必须先捕获。)
            print("Proxy connection failed; retrying the AnySearch endpoint directly.", file=sys.stderr)
            with requests.Session() as direct_session:
                direct_session.trust_env = False
                resp = direct_session.post(ENDPOINT, json=payload, headers=_build_headers(api_key), timeout=30)
