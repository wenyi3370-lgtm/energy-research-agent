...
def convert_text(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    if paragraph_runs is not None:
        ...
        # LibreOffice ignores spAutoFit and wraps text at the frame width
        # (cx) with wrap="square", so multi-line frames get 1.30x headroom
        # to keep every drawn line from re-wrapping at render time.
        frame_headroom = 1.30
    else:
        text_width = estimate_text_width(full_text, font_size, font_weight) * 1.05
        text_height = font_size * 1.5
        # Single-line KPI numbers must never split at render: LibreOffice
        # wraps at the frame width while the coarse 0.55em/0.75em estimate
        # under-measures serif fonts (Georgia digits ~0.62em, M/W ~0.9em),
        # which split "540 MWh" into "540"/"MWh". 1.50x headroom on the
        # frame (NOT on positioning) absorbs the shaping difference;
        # PowerPoint (wrap="none" + spAutoFit) ignores the wider frame.
        frame_headroom = 1.50
    ...
    box_w = (text_width + padding * 2) * frame_headroom
    ...
    # Keep the (headroom-inflated) frame inside the canvas: LibreOffice
    # ignores spAutoFit and wraps/clips at the frame width, so a frame that
    # sticks past the right edge would rewrap right-side texts (end-anchored
    # footers rendered start-aligned by LO land beyond the slide edge).
    canvas_w_emu = px_to_emu(1280.0)
    if off_x < 0:
        off_x = 0
    if off_x + ext_cx > canvas_w_emu:
        ext_cx = max(canvas_w_emu - off_x, px_to_emu(16))
