...
def _resolve_repo_root() -> Path:
    """FIX-08: locate the repo root robustly — never via fixed parent chains.

    Priority: OVERSEAS_ENERGY_SKILL_ROOT env var -> the first ancestor of
    this script that contains a .git directory -> the skill root itself
    (requirements.txt lives at the skill root). Any git mutation is refused
    unless the resolved root is verified to contain .git.
    """
    env = os.environ.get("OVERSEAS_ENERGY_SKILL_ROOT", "").strip()
    if env:
...
def ensure_repo_root_safe() -> None:
    """FIX-08: refuse git operations unless the repo root is verified.

    ABORT unless REPO_ROOT contains .git; warn (but allow) when the
    directory name differs and an explicit env override is in use.
    """
    if not (REPO_ROOT / ".git").exists():
        raise RuntimeError(
            "Repo root %s has no .git — refusing git operations. "
            "Fix OVERSEAS_ENERGY_SKILL_ROOT or the directory layout." % REPO_ROOT
        )
    env_override = bool(os.environ.get("OVERSEAS_ENERGY_SKILL_ROOT", "").strip())
    if not env_override and "overseas-energy-market-research" not in REPO_ROOT.name:
        raise RuntimeError(
            "Repo root %r does not look like overseas-energy-market-research — ABORT (FIX-08)"
            % REPO_ROOT.name
        )
...
def main() -> int:
    ...
    try:
        ...
        ensure_repo_root_safe()  # FIX-08: verify .git + repo identity BEFORE any git command
