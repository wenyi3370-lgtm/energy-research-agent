...
- High-quality cloud mode: provider API key is set before use:
  ...
  - Keys may live in the current process environment or the first `.env` found in this order: current working directory, skill directory (e.g. `~/.agents/${SKILL_DIR}/.env`), clone repo root, `~/.ppt-master/.env`
...
- (If skipped embedding) one-line hint on how to embed later: `python3 ${SKILL_DIR}/scripts/svg_to_pptx.py <project_path> --recorded-narration audio`.
