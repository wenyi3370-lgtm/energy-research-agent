...
metadata:
  ...
  credential: "ANYSEARCH_API_KEY is optional; anonymous access is available, and the key may be supplied by .env, environment variable, or --api_key."
---
...
- On Python, honor the configured/system proxy first. If `requests` raises `ProxyError`, retry the same AnySearch endpoint once with `Session.trust_env = False`; this is a process-local direct retry and must not alter the user's global proxy settings. If that retry fails, continue with the normal runtime fallback.
