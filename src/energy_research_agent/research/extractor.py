...
才允许留空。若页面为主要会计数据表或年度比较表，且列出了多个可比
年度（例如 2022/2023/2024 三年对比），必须为每一个明确写出的年度
分别生成一条 Claim，逐行提取，不得只取最新一年。单点营销数字
（服务热线、服务站数量、页面计数等）不是财务事实，不得提取为
财务字段。
"""
...
class EvidenceExtractor:
    ...
    def _sanitize_batch(
        ...
        """
        from energy_research_agent.research.entity_scope import normalized_entity_name
        from energy_research_agent.research.source_grader import normalize_source_kind
        query_id = result.query_id
