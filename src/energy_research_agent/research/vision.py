...
- ``default_vision_verifier()`` — prefers DeepSeek-V4-Flash-Vision-Exp via the
  research gateway credentials (``ERA_DEEPSEEK_API_KEY``, ≤384 tokens per
  image); dedicated ``ERA_VISION_API_BASE``/``ERA_VISION_API_KEY`` settings
  take precedence; OpenAI fallback uses
  ``ERA_OPENAI_API_KEY``.  Returns ``None`` when nothing is configured.
...
def _load_gateway_config() -> dict[str, Any] | None:
    # 1) optional repository-local gateway configuration
    for parent in Path(__file__).resolve().parents:
        ...
        if config.is_file():
            try:
                ...
                return None
    # 2) reuse the Agent settings — dedicated vision credentials first:
    ...
    try:
        ...
        # key/base (ERA_VISION_API_KEY / ERA_VISION_API_BASE).
...
class GatewayVisionVerifier:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.timeout_seconds = max(
            10, min(45, int(os.getenv("ERA_VISION_TIMEOUT_SECONDS", "30")))
        )
