...
This connects the research pipeline to the automation service
...
kernel, as a deterministic state-driven pipeline:
    ...
    -> deterministic ingest + validation (NO freeze) -> CoreValidator gate

and on the publish side (only ever called after APPROVED):

    finalize (re-validate -> freeze -> plan -> export)
...
class OrchestratingExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        self.fulltext_pages_per_query = max(
            1, int(os.environ.get("ERA_FULLTEXT_PAGES_PER_QUERY", "3"))
        )
        self.fulltext_workers = max(
            1, min(12, int(os.environ.get("ERA_FULLTEXT_WORKERS", "6")))
        )
    ...
    def from_environment(
        ...
        ``ERA_KIMI_WEB_SESSION`` (default ``default``).
        """
        ...
        try:
            resolved["kimi_webbridge"] = KimiWebBridgeAdapter(
                os.environ.get("ERA_KIMI_WEB_SESSION", "default"),
                daemon_url=os.environ.get(
                    "ERA_KIMI_WEB_DAEMON_URL", "http://127.0.0.1:10086"
                ),
            )
    ...
    def _image_discovery_pass(
        ...
        max_pages = max(1, int(os.environ.get("ERA_IMAGE_DISCOVERY_PAGES", "8")))
        ...
        max_candidates = max(1, int(os.environ.get("ERA_IMAGE_DISCOVERY_MAX_CANDIDATES", "24")))
