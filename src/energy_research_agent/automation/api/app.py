"""FastAPI application factory for the Energy Research Agent.

The primary surface is ``/api/agent`` for mission parsing, approval,
execution, continuation, deep research, Trace and artifacts. The module also
exposes stable service endpoints used by controlled executors and integrations:
...
logger = logging.getLogger("energy_research_agent.automation.api")
...
def _default_executor() -> ResearchExecutor:
    """Pick the run executor from ``ERA_AUTOMATION_EXECUTOR``.
...
def _project_root() -> Path:
    ...
    The wheel layout (``/app/src/energy_research_agent/...``) differs
    from the host editable layout (``<repo>/src/energy_research_agent/...``);
...
def _scan_live_acceptance(query: str) -> list[dict]:
    ...
    The host repo is mounted at ERA_SKILL_ROOT inside the container, so the
    deep-research endpoint can continue historical live surveys the same way
    it continues automation runs.
    """
    skill_root = Path(os.environ.get("ERA_SKILL_ROOT", "/skill"))
...
def create_app(
    ...
    ``database_url`` defaults to ``ERA_AUTOMATION_DATABASE_URL`` then
    ``sqlite:///./automation.db``; ``workdir`` defaults to
    ``ERA_AUTOMATION_WORKDIR`` then ``./automation_work``; ``executor``
    ...
    (no-op when ``ERA_FEISHU_*`` is not set).
    """
    db = AutomationDatabase(
        database_url
        or os.environ.get("ERA_AUTOMATION_DATABASE_URL")
    ...
    service = ResearchService(
        ...
        workdir=Path(workdir or os.environ.get("ERA_AUTOMATION_WORKDIR") or "automation_work"),
    ...
    def lookup_research(q: str = "") -> dict:
        ...
        同时扫描宿主机仓库的 build/live_acceptance（挂载于 ERA_SKILL_ROOT），
    ...
    def _run_deep_research(run_id: str, payload: DeepResearchPayload, run_dir: Path) -> None:
        from energy_research_agent.research.deep_retry import deep_retry, find_evidence_store
        ...
        try:
            search_roots = [
                Path(os.environ.get("ERA_SKILL_ROOT", "/skill")) / "build" / "live_acceptance",
                service.workdir,
            ]
            store = None
            if payload.run_dir:
                ...
                legacy_or_current_name = next(
                    (name for name in ("energy-research-agent", "energy-research-agent") if name in str(candidate)),
                    None,
                )
            ...
            if store is None:
                ...
                if not result.get("published") and payload.publish_conditional:
                    ...
                    try:
                        if last_store_path and Path(last_store_path).is_file():
                            from energy_research_agent.research.deep_retry import publish_conditionally
                ...
                if payload.save_to_desktop and result.get("published"):
                    desktop_root = Path(os.environ.get("ERA_DESKTOP_PATH", "/desktop"))
    ...
    def health() -> JSONResponse:
        try:
            ...
            return JSONResponse({
                ...
                "portal_file": str((_PORTAL_DIR / "agent.html").resolve()),
            })
    ...
    def _intelligence_catch_up_watchdog() -> None:
        ...
        # 测试环境可关闭巡检（ERA_INTELLIGENCE_CATCHUP=off），生产默认开启。
        if os.environ.get("ERA_INTELLIGENCE_CATCHUP", "on").strip().lower() != "on":
            return
