...
environment: ``ERA_FEISHU_APP_ID`` / ``ERA_FEISHU_APP_SECRET`` and
``ERA_FEISHU_DEFAULT_RECEIVER``. Without credentials the adapter is
...
logger = logging.getLogger("energy_research_agent.automation.feishu")
...
class LarkFeishuAdapter:
    """Production adapter; requires ERA_FEISHU_APP_ID + ERA_FEISHU_APP_SECRET."""
    ...
    def __init__(
        ...
        self.app_id = app_id or os.environ.get("ERA_FEISHU_APP_ID")
        self.app_secret = app_secret or os.environ.get("ERA_FEISHU_APP_SECRET")
        self.default_receiver = default_receiver or os.environ.get("ERA_FEISHU_DEFAULT_RECEIVER")
    ...
    def send(self, message: FeishuMessage) -> FeishuDelivery:
        if not self.available():
            return FeishuDelivery(
                delivered=False,
                diagnostics=["Feishu not configured: set ERA_FEISHU_APP_ID/SECRET/DEFAULT_RECEIVER"],
            )
